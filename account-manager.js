...
function validName(name) { return /^[a-zA-Z0-9._-]{1,64}$/.test(name); }

// ===== challenge management =====
const challenges = new Map();
const CH_TTL_MS = 10 * 60 * 1000;
setInterval(() => { const now = Date.now(); for (const [id, ch] of challenges) if (now - ch.createdAt > CH_TTL_MS) challenges.delete(id); }, 60 * 1000);
...
export default function registerAccountManager(app) {
  ...
  router.get("/session", async (_req, res) => {
    try {
      const { SESS_DIR } = await ensureDirs();
  ...
  // POST /v1/auth/login/verify_password (SRP flow)
  router.post("/login/verify_password", guard, async (req, res) => {
    try {
      ...
      // SRP handshake (вместо client.checkPassword)
      const pwd = String(password || "");
      const pwdInfo = await client.invoke(new Api.account.GetPassword());
      // eslint-disable-next-line no-underscore-dangle
      const srp = await client._computePasswordSrp(pwd, pwdInfo);
      await client.invoke(new Api.auth.CheckPassword({ password: new Api.InputCheckPasswordSRP({ srp_id: srp.srpId, A: srp.A, M1: srp.M1 }) }));
      ...
      const msg = String(e?.message || e);
      if (msg.includes("SRP_ID_INVALID") || msg.includes("PASSWORD_HASH_INVALID")) return err(res, 400, "BAD_REQUEST", "Invalid 2FA password");
